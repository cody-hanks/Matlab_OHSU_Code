...
lr=0;
    ...
    dataset = [];
        ...
        load('DataSheet5.mat');
        DataSheet(1:48,4:24) = (coeff'*(DataSheet(1:48,4:24)-repmat(mu,48,1))')';
        ...
        correctlv1 =correctlv1 + sum(DataSheet(testset,31)==predictionl1);
        
     for pred = 1:length(predictionl1)
            if (predictionl1(pred) ==1 )
                if predictionl1(pred) == DataSheet(testset(pred),31)
                    fb =  fb +1;
                end
                pred1 = predict(mdlfb,DataSheet(testset(pred),featurel2));
                if(pred1 == DataSheet(testset(pred),32))
            ...
            elseif (predictionl1(pred) ~= 1)
                ...
                if(pred2 == DataSheet(testset(pred),32))
