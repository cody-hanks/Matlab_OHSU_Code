...
path = ['/home/cody/Documents/for_cody_11_30/POCL_6308_Data/', 'POCL_ID_6308_'];
...
% old load cell location 
UR_L = [0, 55].*25.4;
UL_L = [50, 55].*25.4;
LR_L = [0, 0].*25.4;
LL_L = [50, 0].*25.4;

co_ords = [UR_L; UL_L; LR_L; LL_L];
...
'R stom'];
    ...
    labels = ['BCK';'LFT';'RHT';'STM';'RHT_L  ';'STM_L  '; ...
...
positionval_old_FB_LR = [1;2;2;1];
positionval_old_F_B_L_R = [1;2;3;4];
...
%         subind = two_subjects_time_indices(kkkk,1);
    ...
    for j = dataset(1:end)
        ...
        DataSheet(j,31) = positionval_old_FB_LR(ipos);
        DataSheet(j,32) = positionval_old_F_B_L_R(ipos);
        ipos = ipos +1;
        if (ipos >positions) 
            ipos = 1;
        end
    ...
    file_LC = [path, num2str(subind), '/POCL_ID_6308_', num2str(subind), '_data_NEWT.mat'];
    load(file_LC)
    
    data = [raw_ur_M,raw_ul_M,raw_lr_M,raw_ll_M];
    %%%%%%%%%%%%%%%%%%
    % read XLS file 
...
%    [B,A] = cheby2(N,40,Ws);
    ...
    bck = floor(4000/250);
    rht = floor(1954000/250);
    lft = floor(994000/250);
    stm = floor(2914000/250);
    
    
    % get start and stop time for each segment get a combo of all segs and
    % each seg ind.
    stoptime = 3000;%floor((100*(250/devid))/2);%2*60*2-1-(2*15);
    %seg_back_cal_L_25 = floor(nums(2,1)/devid)+stoptime:floor(nums(2,1)/devid)+stoptime+stoptime; %1
    %seg_back_cal_R_25 = floor(nums(3,1)/devid)+stoptime:floor(nums(3,1)/devid)+stoptime+stoptime; %2
    %seg_bck_L_25 = floor(nums(4,1)/devid)+75+stoptime:floor(nums(4,1)/devid)+75+stoptime+stoptime;     %3
    %seg_lft_L_25 = floor(nums(5,1)/devid)+75+stoptime:floor(nums(5,1)/devid)+75+stoptime+stoptime;     %4
    %seg_rht_L_25 = floor(nums(6,1)/devid)+75+stoptime:floor(nums(6,1)/devid)+75+stoptime+stoptime;     %5
    %seg_stm_L_25 = floor(nums(7,1)/devid)+75+stoptime:floor(nums(7,1)/devid)+75+stoptime+stoptime;     %6
    %seg_bck_R_25 = floor(nums(8,1)/devid)+75+stoptime:floor(nums(8,1)/devid)+75+stoptime+stoptime;     %7
    %seg_lft_R_25 = floor(nums(9,1)/devid)+75+stoptime:floor(nums(9,1)/devid)+75+stoptime+stoptime;     %8
    %seg_rht_R_25 = floor(nums(10,1)/devid)+75+stoptime:floor(nums(10,1)/devid)+75+stoptime+stoptime;     %9
    %seg_stm_R_25 = floor(nums(11,1)/devid)+75+stoptime:floor(nums(11,1)/devid)+75+stoptime+stoptime;     %10
    seg_bck_25 = floor(4000/devid):floor(4000/devid)+stoptime;
    seg_lft_25 = floor(994000/devid):floor(994000/devid)+stoptime;
    seg_rht_25 = floor(1954000/devid):floor(1954000/devid)+stoptime;
    seg_stm_25 = floor(2914000/devid):floor(2914000/devid)+stoptime;
    
    %reset the stop time 
    stoptime = 240;

    
    %seg_bck_cal_L = [back_cal_L+stoptime:back_cal_L+stoptime+stoptime];
    %seg_bck_cal_R = [back_cal_R+stoptime:back_cal_R+stoptime+stoptime];
    %seg_bck_L = [bck_L+stoptime:bck_L+stoptime+stoptime];
    %seg_bck_R = [bck_R+stoptime:bck_R+stoptime+stoptime];
    %seg_lft_L = [lft_L+stoptime:lft_L+stoptime+stoptime];
    %seg_lft_R = [lft_R+stoptime:lft_R+stoptime+stoptime];
    %seg_rht_L = [rht_L+stoptime:rht_L+stoptime+stoptime];
    %seg_rht_R = [rht_R+stoptime:rht_R+stoptime+stoptime];
    %seg_stm_L = [stm_L+stoptime:stm_L+stoptime+stoptime];
    %seg_stm_R = [stm_R+stoptime:stm_R+stoptime+stoptime];
    seg_bck = [bck:bck+stoptime];
    seg_lft =[lft:lft+stoptime];
    seg_rht =[rht:rht+stoptime];
    seg_stm=[stm:stm+stoptime];
    
    
      
%     all_periods = [ back_cal_L+stoptime:back_cal_L+stoptime+stoptime, ...
%         back_cal_R+stoptime:back_cal_R+stoptime+stoptime, ...
%         bck_L+stoptime:bck_L+stoptime+stoptime,lft_L+stoptime:lft_L+stoptime+stoptime,rht_L+stoptime:rht_L+stoptime+stoptime,stm_L+stoptime:stm_L+stoptime+stoptime, ...
%         bck_R+stoptime:bck_R+stoptime+stoptime,lft_R+stoptime:lft_R+stoptime+stoptime,rht_R+stoptime:rht_R+stoptime+stoptime,stm_R+stoptime:stm_R+stoptime+stoptime];
%     all_periods_it = [ back_cal_L+stoptime:back_cal_L+stoptime+stoptime; ...
%         back_cal_R+stoptime:back_cal_R+stoptime+stoptime; ...
%         bck_L+stoptime:bck_L+stoptime+stoptime;lft_L+stoptime:lft_L+stoptime+stoptime;rht_L+stoptime:rht_L+stoptime+stoptime;stm_L+stoptime:stm_L+stoptime+stoptime; ...
%         bck_R+stoptime:bck_R+stoptime+stoptime;lft_R+stoptime:lft_R+stoptime+stoptime;rht_R+stoptime:rht_R+stoptime+stoptime;stm_R+stoptime:stm_R+stoptime+stoptime];
%    
...
%         seg_bck_R_25;seg_lft_R_25;seg_rht_R_25;seg_stm_R_25];
    ...
    all_periods = [seg_bck,seg_lft,seg_rht,seg_stm];
    all_periods_it = [seg_bck;seg_lft;seg_rht;seg_stm];
    all_periods_it_25 = [seg_bck_25;seg_lft_25;seg_rht_25;seg_stm_25];
    ...
    dec_cop_x = zeros(241,10);
    dec_cop_y = zeros(241,10);
    for j =1:positions
